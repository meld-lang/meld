/// @file error_propagation_pass.cpp
/// @brief Semantic Analyzer — Error-Propagation Operator Pass implementation
...
ErrorPropagationPass::ErrorPropagationPass() = default;

// ---------------------------------------------------------------------------
// Static helpers
// ---------------------------------------------------------------------------

const std::unordered_set<std::string>& ErrorPropagationPass::forbidden_operator_symbols() {
...
bool ErrorPropagationPass::is_optional_type(const parser::ast::type_annotation& type) {
    if (type.is_nullable) return true;
    if (type.type_name.name == "optional" && type.has_type_arguments) return true;
    return false;
}

bool ErrorPropagationPass::is_result_type(const parser::ast::type_annotation& type) {
...
void ErrorPropagationPass::scan_expression(
    ...
    meld::compat::visit([&](const auto& node) {
        ...
        if constexpr (std::is_same_v<T,
            ...
            check_error_propagation(node.get(), source_file, result,
...
void ErrorPropagationPass::scan_block(
...
// ---------------------------------------------------------------------------
// Type checking for error-propagation operator
// ---------------------------------------------------------------------------

void ErrorPropagationPass::check_error_propagation(
    ...
    result.error_propagations_checked++;

    // If not inside a function body, emit E4706 (no enclosing function)
    if (!in_function) {
    ...
    // If enclosing function has a return type, verify it's Result-compatible
    if (func_return_type && !is_result_type(*func_return_type)) {
        emit_incompatible_return_type_error(
...
void ErrorPropagationPass::check_operator_definition(
...
void ErrorPropagationPass::check_custom_operator_definition(
...
ErrorPropagationResult ErrorPropagationPass::run(
...
void ErrorPropagationPass::emit_non_result_error(
    ...
    std::string message =
        "error-propagation operator '?!' used on type '" + type_name +
        "' which is not Result[T, E] — "
        "the postfix '?!' operator targets Result[T, E] only; "
        "use '?' for optional[T] nil propagation";
    result.diagnostics.push_back(ErrorPropagationDiagnostic{
        .level = ErrorPropagationDiagnostic::Level::Error,
        .code = "E4705",
        .message = message,
...
void ErrorPropagationPass::emit_incompatible_return_type_error(
    ...
    std::string message =
        "error-propagation operator '?!' used in function with return type '" +
        return_type_name + "' which is not compatible with Result[_, E] — "
        "the enclosing function must return Result[_, E] to use the '?!' operator, "
        "because '?!' returns Err(e) from the function when the operand is Err";
    result.diagnostics.push_back(ErrorPropagationDiagnostic{
        .level = ErrorPropagationDiagnostic::Level::Error,
        .code = "E4706",
        .message = message,
...
void ErrorPropagationPass::emit_forbidden_overload_error(
    ...
    std::string message =
        ...
        "the error-propagation operator has fixed compiler-guaranteed semantics "
        "to ensure predictable error-propagation control flow; it cannot be redefined";
    result.diagnostics.push_back(ErrorPropagationDiagnostic{
        .level = ErrorPropagationDiagnostic::Level::Error,
        .code = "E4702",
        .message = message,
